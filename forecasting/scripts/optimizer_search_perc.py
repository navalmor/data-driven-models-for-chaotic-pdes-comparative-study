...
logger = get_logger("forecasting.scripts.optimizer_search_perc")

OPTIMIZER_NAMES: tuple[str, ...] = ("gp", "gbrt", "forest", "dummy")
...
FIELDNAMES = [
    ...
    "constraint_train_max_abs_violation_model_space",
    "constraint_train_mean_abs_violation_model_space",
    "validation_constraint_physical_max_abs_error",
    "constraint_type",
    "constraint_enforced_space",
    "is_exact_physical_constraint",
    "physical_constraint_evaluated_after_decoding",
    "surrogate_slope_norm",
    "surrogate_decoded_train_integral_std",
    "surrogate_train_rmse",
    "surrogate_train_mean_abs_error",
    "surrogate_train_max_abs_error",
    "decoder_reconstruction_integral_rmse",
    "objective_value",
    "summary_path",
    "output_dir",
    "error",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run validation-only PERC optimizer search."
    )
    parser.add_argument(
        "--config",
        required=True,
        type=Path,
        help="Path to a JSON PERC optimizer-search config.",
    )
...
def _native_dimension_for_mode(data_mode: str) -> int:
    if data_mode == "full64":
        return 64
    if data_mode == "latent8":
        return 8
    raise ValueError(
        "PERC optimizer search supports data_mode='full64' or data_mode='latent8', "
        f"got {data_mode!r}."
    )


def _feature_dimension(candidate: dict[str, Any], *, data_mode: str) -> int:
    input_dim = _native_dimension_for_mode(data_mode)
...
def _build_single_config(
    *,
    ...
    return {
        ...
        "constraint": dict(require_section(optimizer_cfg, "constraint")),
        "model": {
        ...
        "metadata": {
            "stage": f"perc_{data_mode}_optimizer_search",
            "optimizer_search_run_id": search_run_id,
            "optimizer": optimizer_name,
            "trial": trial,
            "selection_policy": "validation_only_optimizer_search_not_final_test_evaluation",
        },
    }
...
def _row_from_result(
    *,
    ...
    params = summary["perc_parameters"]
    valid_constraint = summary.get("validation_constraint_physical") or {}
    constraint = summary.get("perc_constraint") or {}
    surrogate_fit = constraint.get("surrogate_fit") or {}
    surrogate_train_error = surrogate_fit.get("train_error") or {}
    decoder_integral_error = constraint.get("decoder_reconstruction_integral_error") or {}
    ...
    return {
        ...
        "constraint_train_max_abs_violation_model_space": summary.get("constraint_train_max_abs_violation_model_space"),
        "constraint_train_mean_abs_violation_model_space": summary.get("constraint_train_mean_abs_violation_model_space"),
        "validation_constraint_physical_max_abs_error": valid_constraint.get("max_abs_error"),
        "constraint_type": constraint.get("type"),
        "constraint_enforced_space": constraint.get("constraint_enforced_space"),
        "is_exact_physical_constraint": constraint.get("is_exact_physical_constraint"),
        "physical_constraint_evaluated_after_decoding": constraint.get("physical_constraint_evaluated_after_decoding"),
        "surrogate_slope_norm": surrogate_fit.get("slope_norm"),
        "surrogate_decoded_train_integral_std": surrogate_fit.get("decoded_train_integral_std"),
        "surrogate_train_rmse": surrogate_train_error.get("rmse"),
        "surrogate_train_mean_abs_error": surrogate_train_error.get("mean_abs_error"),
        "surrogate_train_max_abs_error": surrogate_train_error.get("max_abs_error"),
        "decoder_reconstruction_integral_rmse": decoder_integral_error.get("rmse"),
...
def main() -> int:
    ...
    try:
        ...
        configure_from_args(args, component="OPT-PERC",
                            config_level=_get_logging_level_from_config(cfg))
        validate_experiment(cfg, expected_type="perc_optimizer_search", expected_model="perc")
        ...
        logger.info(
            "PERC optimizer search start | run=%s mode=%s optimizers=%s",
            search_run_id,
            data_mode,
        ...
        logger.info(
            "PERC optimizer search done | run=%s best_optimizer=%s best_run=%s valid_h=%s",
            search_run_id,
        ...
        logger.exception("PERC optimizer search failed")
        return 1
