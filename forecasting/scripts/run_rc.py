...
logger = get_logger("forecasting.scripts.run_rc")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one config-driven RC forecasting experiment."
    )
    parser.add_argument(
        "--config",
        required=True,
        type=Path,
        help="Path to the final JSON RC experiment config.",
    )
...
def main() -> int:
    ...
    # Configure logging up front (component RC) so a failure while loading the
    ...
    configure_from_args(args, component="RC")

    try:
        cfg = load_config(args.config)
        configure_from_args(args, component="RC", config_level=_get_logging_level_from_config(cfg))
